"""
evaluate_exploration.py — Evaluation and Visualization for Partially Observable Snake

This module evaluates trained Snake agents and generates comparative visualizations:
1. Baseline agent (rule-based explorer)
...
All evaluations run on the target 7×7 board with partial observability (3×3 FOV).

Visualization Outputs:
- comparison_results_po.png: Performance curves for all evaluated agents with inset zoom
- training_progress_po.png: Phase 1 (7×7 board) training curve with fitted trend
"""
...
from environments_partially_observable import OriginalSnakeEnvironment
from snake_model import ActorCriticModel, GRU_UNITS
from baseline_exploration import SquareExplorerAgent
...
def run_simulation(agent_type, agent_obj=None, weight_path=None):
    ...
    Args:
        ...
        agent_obj: SquareExplorerAgent instance (for Baseline)
    ...
    """
    env   = OriginalSnakeEnvironment(NUM_EVAL_BOARDS, BOARD_SIZE, MASK_SIZE)
    model = None
    ...
    if agent_type == "RL":
        ...
        # Initialize GRU hidden states (persistent across steps)
        hidden_states = np.zeros((NUM_EVAL_BOARDS, GRU_UNITS), dtype=np.float32)
    ...
    for _ in tqdm(range(EVAL_STEPS), desc=desc, leave=False):
        ...
        if agent_type == "RL":
            ...
            # Forward pass (3-tuple return: logits, value, new_hidden)
            logits, _, new_hidden = model(state_tensor,
            ...
            action_env = agent_obj.predict(env)
        ...
        # Update baseline agent's internal state
        if agent_type == "Baseline":
            for i in range(NUM_EVAL_BOARDS):
                agent_obj.internal_bodies[i] = env.bodies[i]
...
def plot_training_history(baseline_score=None):
    """
    Plot training progress for Phase 1 (7×7 board) with fitted trend curve.
    
    Reads training_history_po.json and filters to curriculum phase 1 data.
    Generates training_progress_po.png.
    """
    history_path = "training_history_po.json"
    ...
    try:
        ...
        phases  = np.array(data.get("curriculum_phase", [0] * len(steps)))
    except Exception as e:
        print(f"Error reading history file: {e}")
        return

    # Filter to Phase 1 (7×7 board, the target task)
    mask = phases == 1
    if not np.any(mask):
        print("No Phase 1 data found in training history.")
        return
    
    s, r = steps[mask], rewards[mask]

    plt.figure(figsize=(12, 6))
    
    # Raw data as scatter points
    plt.scatter(s, r, color='steelblue', s=20, alpha=0.4,
                label='Average reward per update')
    
    # Fitted polynomial trend
    if len(s) > 10:
        degree = min(21, len(s) - 1)
        try:
            z = np.polyfit(s, r, degree)
            p = np.poly1d(z)
    ...
    plt.tight_layout()
    plt.savefig("training_progress_po.png", bbox_inches="tight")
    print("Graph saved as 'training_progress_po.png'")
    plt.show()
...
def main():
    ...
    Evaluates all available agents and generates comparison plot.
    """
    ...
    # 1. Baseline Explorer
    print("\n=== Baseline Explorer Evaluation ===")
    baseline_agent = SquareExplorerAgent(NUM_EVAL_BOARDS)
    ...
    best_score, best_history = run_simulation(
        "RL", weight_path="snake_po_weights_best.weights.h5"
    )
    ...
    # 3. Final RL weights
    print("\n=== Final Weights Evaluation ===")
    final_score, final_history = run_simulation(
        "RL", weight_path="snake_po_weights_final.weights.h5"
    )
    if final_score is not None:
        results["Final"] = (final_score, final_history)
        print(f"  Score: {final_score:.2f}")

    # 4. All periodic snapshots
    print("\n=== Periodic Snapshot Evaluation ===")
    snapshot_files = sorted(glob.glob("snake_po_weights_step*.weights.h5"))
    ...
    # Build plot order: Baseline → Final → Best → Step checkpoints (descending)
    plot_order = []
    ...
    ax.legend(fontsize=17, ncol=2, framealpha=1.0, loc='upper left',
    ...
    plt.tight_layout()
    plt.savefig("comparison_results_po.png", bbox_inches="tight", dpi=120)
    print("Comparison graph saved as 'comparison_results_po.png'")
    plt.show()
